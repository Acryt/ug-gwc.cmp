<?php
	wp_enqueue_style( 'paccrd', URI . '/assets/paccrd.bundle.css' );
?>
<section id="priceAccrd" class="section price_accrd accrd-js">
	<div class="wrapper">
		<div class="section__header">
			<h2>Ghostwriter Preise pro Seite</h2>
		</div>
		<div class="section__content">
			<div class="price_accrd__cont shadow">
				<div class="price_accrd__nav">
					<button class="accrd-b price_accrd__btn _active">Geisteswissenschaften</button>
					<button class="accrd-b price_accrd__btn">Wirtschaftswissenschaften</button>
					<button class="accrd-b price_accrd__btn">Ingenieurwissenschaften</button>
					<button class="accrd-b price_accrd__btn">Rechtswissenschaften</button>
					<button class="accrd-b price_accrd__btn">Medizin und Naturwissenschaften</button>
				</div>
				<div class="accrd-m price_accrd__desc _active" itemscope itemtype="https://schema.org/Table">
					<table class="price_accrd__table">
						<tbody>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Hausarbeit">
								<td>Hausarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bachelorarbeit">
								<td>Bachelorarbeit</td>
								<td class="price__plus">ab
									<span>51 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Masterarbeit">
								<td>Masterarbeit</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Diplomarbeit">
								<td>Diplomarbeit</td>
								<td class="price__plus">ab
									<span>51 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Doktorarbeit">
								<td>Doktorarbeit</td>
								<td class="price__plus">ab
									<span>62 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Facharbeit">
								<td>Facharbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Abschlussarbeit">
								<td>Abschlussarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bewerbung">
								<td>Bewerbung</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Biografie">
								<td>Biografie</td>
								<td class="price__plus">ab
									<span>49 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Businessplan">
								<td>Businessplan</td>
								<td class="price__plus">ab
									<span>59 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Danksagung">
								<td>Danksagung</td>
								<td class="price__plus">ab
									<span>40 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Essay">
								<td>Essay</td>
								<td class="price__plus">ab
									<span>49 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Exposé">
								<td>Exposé</td>
								<td class="price__plus">ab
									<span>51 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Forschungsartikel">
								<td>Forschungsartikel</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Motivationsschreiben">
								<td>Motivationsschreiben</td>
								<td class="price__plus">ab
									<span>150 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Präsentationen">
								<td>Präsentationen</td>
								<td class="price__plus">ab
									<span>25 €</span> PRO DIA
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Projektarbeit">
								<td>Projektarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Referat">
								<td>Referat</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Seminararbeit">
								<td>Seminararbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Analyse">
								<td>Statistische Analyse</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Auswertung">
								<td>Statistische Auswertung</td>
								<td class="price__plus">ab
									<span>120 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Studienarbeit">
								<td>Studienarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Zusammenfassung">
								<td>Zusammenfassung</td>
								<td class="price__plus">ab
									<span>60 €</span> PRO SEITE
								</td>
							</tr>
						</tbody>
					</table>
					<ul class="price_accrd__pros">
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">
							Zahlreiche Autoren aus verschiedenen Bereichen der Geisteswissenschaften</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							Ihrer
							Arbeit nach Ihren Wünschen und Anforderungen</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							des
							anonymen direkten Kontakts mit dem Autor</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							sowohl der ganzen Arbeit als auch einzelner Teile</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Unbefristete
							Garantie für die Korrektur der Arbeit*</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Inhaltsverzeichnis, Literaturverzeichnis, Abbildungsverzeichnis - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Plagiatsprüfung
							- KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit von einem unabhängigen Korrekturleser</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit in der Qualitätskontrolleabteilung - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Rechtzeitige
							Teillieferungen der Arbeit</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							der
							Teilzahlung ohne Zusatzkosten</li>
					</ul>
				</div>
				<div class="accrd-m price_accrd__desc" itemscope itemtype="https://schema.org/Table">
					<table class="price_accrd__table">
						<tbody>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Hausarbeit"> 
								<td>Hausarbeit</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bachelorarbeit"> 
								<td>Bachelorarbeit</td>
								<td class="price__plus">ab
									<span>53 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Masterarbeit"> 
								<td>Masterarbeit</td>
								<td class="price__plus">ab
									<span>54 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Diplomarbeit"> 
								<td>Diplomarbeit</td>
								<td class="price__plus">ab
									<span>53 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Doktorarbeit"> 
								<td>Doktorarbeit</td>
								<td class="price__plus">ab
									<span>64 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Facharbeit"> 
								<td>Facharbeit</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Abschlussarbeit"> 
								<td>Abschlussarbeit</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bewerbung"> 
								<td>Bewerbung</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Biografie"> 
								<td>Biografie</td>
								<td class="price__plus">ab
									<span>49 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Businessplan"> 
								<td>Businessplan</td>
								<td class="price__plus">ab
									<span>59 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Danksagung"> 
								<td>Danksagung</td>
								<td class="price__plus">ab
									<span>40 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Essay"> 
								<td>Essay</td>
								<td class="price__plus">ab
									<span>51 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Exposé"> 
								<td>Exposé</td>
								<td class="price__plus">ab
									<span>53 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Forschungsartikel"> 
								<td>Forschungsartikel</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Motivationsschreiben"> 
								<td>Motivationsschreiben</td>
								<td class="price__plus">ab
									<span>150 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Präsentationen"> 
								<td>Präsentationen</td>
								<td class="price__plus">ab
									<span>25 €</span> PRO DIA
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Projektarbeit"> 
								<td>Projektarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Referat"> 
								<td>Referat</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Seminararbeit"> 
								<td>Seminararbeit</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Analyse"> 
								<td>Statistische Analyse</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Auswertung"> 
								<td>Statistische Auswertung</td>
								<td class="price__plus">ab
									<span>120 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Studienarbeit"> 
								<td>Studienarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Zusammenfassung"> 
								<td>Zusammenfassung</td>
								<td class="price__plus">ab
									<span>60 €</span> PRO SEITE
								</td>
							</tr>
						</tbody>
					</table>
					<ul class="price_accrd__pros">
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">
							Autoren aus allen Bereichen der Wirtschaftswissenschaften</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							Ihrer
							Arbeit nach Ihren Wünschen und Anforderungen</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							des
							anonymen direkten Kontakts mit dem Autor</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							sowohl der ganzen Arbeit als auch einzelner Teile</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Unbefristete
							Garantie für die Korrektur der Arbeit*</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Inhaltsverzeichnis, Literaturverzeichnis, Abbildungsverzeichnis - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Plagiatsprüfung
							- KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit von einem unabhängigen Korrekturleser</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit in der Qualitätskontrolleabteilung - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Rechtzeitige
							Teillieferungen der Arbeit</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							der
							Teilzahlung ohne Zusatzkosten</li>
					</ul>
				</div>
				<div class="accrd-m price_accrd__desc" itemscope itemtype="https://schema.org/Table">
					<table class="price_accrd__table">
						<tbody>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Hausarbeit"> 
								<td>Hausarbeit</td>
								<td class="price__plus">ab
									<span>58 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bachelorarbeit"> 
								<td>Bachelorarbeit</td>
								<td class="price__plus">ab
									<span>58 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Masterarbeit"> 
								<td>Masterarbeit</td>
								<td class="price__plus">ab
									<span>60 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Diplomarbeit"> 
								<td>Diplomarbeit</td>
								<td class="price__plus">ab
									<span>59 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Doktorarbeit"> 
								<td>Doktorarbeit</td>
								<td class="price__plus">ab
									<span>67 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Facharbeit"> 
								<td>Facharbeit</td>
								<td class="price__plus">ab
									<span>58 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Abschlussarbeit"> 
								<td>Abschlussarbeit</td>
								<td class="price__plus">ab
									<span>58 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bewerbung"> 
								<td>Bewerbung</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Biografie"> 
								<td>Biografie</td>
								<td class="price__plus">ab
									<span>49 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Businessplan"> 
								<td>Businessplan</td>
								<td class="price__plus">ab
									<span>59 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Danksagung"> 
								<td>Danksagung</td>
								<td class="price__plus">ab
									<span>40 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Essay"> 
								<td>Essay</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Exposé"> 
								<td>Exposé</td>
								<td class="price__plus">ab
									<span>58 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Forschungsartikel"> 
								<td>Forschungsartikel</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Motivationsschreiben"> 
								<td>Motivationsschreiben</td>
								<td class="price__plus">ab
									<span>150 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Präsentationen"> 
								<td>Präsentationen</td>
								<td class="price__plus">ab
									<span>25 €</span> PRO DIA
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Projektarbeit"> 
								<td>Projektarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Referat"> 
								<td>Referat</td>
								<td class="price__plus">ab
									<span>57 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Seminararbeit"> 
								<td>Seminararbeit</td>
								<td class="price__plus">ab
									<span>58 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Analyse"> 
								<td>Statistische Analyse</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Auswertung"> 
								<td>Statistische Auswertung</td>
								<td class="price__plus">ab
									<span>120 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Studienarbeit"> 
								<td>Studienarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Zusammenfassung"> 
								<td>Zusammenfassung</td>
								<td class="price__plus">ab
									<span>60 €</span> PRO SEITE
								</td>
							</tr>
						</tbody>
					</table>
					<ul class="price_accrd__pros">
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">
							Arbeit mit speziellen Programmen und Software</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							Ihrer
							Arbeit nach Ihren Wünschen und Anforderungen</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							des
							anonymen direkten Kontakts mit dem Autor</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							sowohl der ganzen Arbeit als auch einzelner Teile</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Unbefristete
							Garantie für die Korrektur der Arbeit*</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Inhaltsverzeichnis, Literaturverzeichnis, Abbildungsverzeichnis - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Plagiatsprüfung
							- KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit von einem unabhängigen Korrekturleser</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit in der Qualitätskontrolleabteilung - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Rechtzeitige
							Teillieferungen der Arbeit</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							der
							Teilzahlung ohne Zusatzkosten</li>
					</ul>
				</div>
				<div class="accrd-m price_accrd__desc" itemscope itemtype="https://schema.org/Table">
					<table class="price_accrd__table">
						<tbody>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Hausarbeit"> 
								<td>Hausarbeit</td>
								<td class="price__plus">ab
									<span>55 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bachelorarbeit"> 
								<td>Bachelorarbeit</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Masterarbeit"> 
								<td>Masterarbeit</td>
								<td class="price__plus">ab
									<span>59 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Diplomarbeit"> 
								<td>Diplomarbeit</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Doktorarbeit"> 
								<td>Doktorarbeit</td>
								<td class="price__plus">ab
									<span>66 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Facharbeit"> 
								<td>Facharbeit</td>
								<td class="price__plus">ab
									<span>55 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Abschlussarbeit"> 
								<td>Abschlussarbeit</td>
								<td class="price__plus">ab
									<span>55 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bewerbung"> 
								<td>Bewerbung</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Biografie"> 
								<td>Biografie</td>
								<td class="price__plus">ab
									<span>49 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Businessplan"> 
								<td>Businessplan</td>
								<td class="price__plus">ab
									<span>59 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Danksagung"> 
								<td>Danksagung</td>
								<td class="price__plus">ab
									<span>40 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Essay"> 
								<td>Essay</td>
								<td class="price__plus">ab
									<span>55 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Exposé"> 
								<td>Exposé</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Forschungsartikel"> 
								<td>Forschungsartikel</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Motivationsschreiben"> 
								<td>Motivationsschreiben</td>
								<td class="price__plus">ab
									<span>150 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Präsentationen"> 
								<td>Präsentationen</td>
								<td class="price__plus">ab
									<span>25 €</span> PRO DIA
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Projektarbeit"> 
								<td>Projektarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Referat"> 
								<td>Referat</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Seminararbeit"> 
								<td>Seminararbeit</td>
								<td class="price__plus">ab
									<span>55 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Analyse"> 
								<td>Statistische Analyse</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Auswertung"> 
								<td>Statistische Auswertung</td>
								<td class="price__plus">ab
									<span>120 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Studienarbeit"> 
								<td>Studienarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Zusammenfassung"> 
								<td>Zusammenfassung</td>
								<td class="price__plus">ab
									<span>60 €</span> PRO SEITE
								</td>
							</tr>
						</tbody>
					</table>
					<ul class="price_accrd__pros">
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">
							Unsere Autoren arbeiten in allen Rechtsbereichen</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							Ihrer
							Arbeit nach Ihren Wünschen und Anforderungen</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							des
							anonymen direkten Kontakts mit dem Autor</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							sowohl der ganzen Arbeit als auch einzelner Teile</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Unbefristete
							Garantie für die Korrektur der Arbeit*</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Inhaltsverzeichnis, Literaturverzeichnis, Abbildungsverzeichnis - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Plagiatsprüfung
							- KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit von einem unabhängigen Korrekturleser</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit in der Qualitätskontrolleabteilung - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Rechtzeitige
							Teillieferungen der Arbeit</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							der
							Teilzahlung ohne Zusatzkosten</li>
					</ul>
				</div>
				<div class="accrd-m price_accrd__desc" itemscope itemtype="https://schema.org/Table">
					<table class="price_accrd__table">
						<tbody>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Hausarbeit"> 
								<td>Hausarbeit</td>
								<td class="price__plus">ab
									<span>51 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bachelorarbeit"> 
								<td>Bachelorarbeit</td>
								<td class="price__plus">ab
									<span>53 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Masterarbeit"> 
								<td>Masterarbeit</td>
								<td class="price__plus">ab
									<span>55 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Diplomarbeit"> 
								<td>Diplomarbeit</td>
								<td class="price__plus">ab
									<span>55 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Doktorarbeit"> 
								<td>Doktorarbeit</td>
								<td class="price__plus">ab
									<span>64 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Facharbeit"> 
								<td>Facharbeit</td>
								<td class="price__plus">ab
									<span>53 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Abschlussarbeit"> 
								<td>Abschlussarbeit</td>
								<td class="price__plus">ab
									<span>54 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Bewerbung"> 
								<td>Bewerbung</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Biografie"> 
								<td>Biografie</td>
								<td class="price__plus">ab
									<span>49 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Businessplan"> 
								<td>Businessplan</td>
								<td class="price__plus">ab
									<span>59 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Danksagung"> 
								<td>Danksagung</td>
								<td class="price__plus">ab
									<span>40 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Essay"> 
								<td>Essay</td>
								<td class="price__plus">ab
									<span>54 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Exposé"> 
								<td>Exposé</td>
								<td class="price__plus">ab
									<span>53 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Forschungsartikel"> 
								<td>Forschungsartikel</td>
								<td class="price__plus">ab
									<span>56 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Motivationsschreiben"> 
								<td>Motivationsschreiben</td>
								<td class="price__plus">ab
									<span>150 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Präsentationen"> 
								<td>Präsentationen</td>
								<td class="price__plus">ab
									<span>25 €</span> PRO DIA
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Projektarbeit"> 
								<td>Projektarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Referat"> 
								<td>Referat</td>
								<td class="price__plus">ab
									<span>54 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Seminararbeit"> 
								<td>Seminararbeit</td>
								<td class="price__plus">ab
									<span>51 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Analyse"> 
								<td>Statistische Analyse</td>
								<td class="price__plus">ab
									<span>52 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Statistische Auswertung"> 
								<td>Statistische Auswertung</td>
								<td class="price__plus">ab
									<span>120 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Studienarbeit"> 
								<td>Studienarbeit</td>
								<td class="price__plus">ab
									<span>50 €</span> PRO SEITE
								</td>
							</tr>
							<tr class="js_btn" data-slr=".popup__bigpromo" data-type="Zusammenfassung"> 
								<td>Zusammenfassung</td>
								<td class="price__plus">ab
									<span>60 €</span> PRO SEITE
								</td>
							</tr>
						</tbody>
					</table>
					<ul class="price_accrd__pros">
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">
							Autoren mit verschiedenen Abschlüssen aus allen medizinischen Bereichen</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							Ihrer
							Arbeit nach Ihren Wünschen und Anforderungen</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							des
							anonymen direkten Kontakts mit dem Autor</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Verfassen
							sowohl der ganzen Arbeit als auch einzelner Teile</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Unbefristete
							Garantie für die Korrektur der Arbeit*</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Inhaltsverzeichnis, Literaturverzeichnis, Abbildungsverzeichnis - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Plagiatsprüfung
							- KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit von einem unabhängigen Korrekturleser</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Überprüfung
							Ihrer Arbeit in der Qualitätskontrolleabteilung - KOSTENLOS</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg"
								alt="check">Rechtzeitige
							Teillieferungen der Arbeit</li>
						<li><img src="<?php echo get_bloginfo('template_url'); ?>/assets/images/icons/check.svg" alt="check">Möglichkeit
							der
							Teilzahlung ohne Zusatzkosten</li>
					</ul>
				</div>
			</div>
		</div>
	</div>
</section>