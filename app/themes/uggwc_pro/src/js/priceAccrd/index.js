export function accrd() {
	const accrds = document.querySelectorAll(".accrd-js");
	accrds.forEach((el) => {
		// Массивы содержащие все переключатели и Меню (обязательно должны быть в одинаковом колличестве)
		const dropToggler = el.querySelectorAll(".accrd-b");
		const dropMenu = el.querySelectorAll(".accrd-m");
